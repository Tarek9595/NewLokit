          ...
          <div className="flex flex-col gap-11.25 w-full ">
            ...
            {groupedCart.length > 0 ? (
              groupedCart.map((group) => (
                <div
                  key={group.id}
                  className="flex flex-col md:flex-row gap-7 justify-between md:items-start p-4 rounded-2xl border border-gray-100 bg-white shadow-sm"
                >
                  <div className="flex gap-4 items-start w-full">
                    <Link
                      to={`/product/${group.id}/detail`}
                    ...
                    <div className="flex flex-col w-full gap-4">
                      <h1 className="text-[14px] line-clamp-2 font-bold text-darky">
                        {group.productName}
                      </h1>

                      <div className="flex flex-col gap-3 bg-[#F6F6F6]/40 p-3 rounded-xl border border-gray-50">
                        {group.variants.map((variant, index) => (
                          <div
                            key={`${variant.selectedSize}-${variant.selectedColor}-${index}`}
                            className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-3 border-b border-gray-200/60 last:border-b-0 last:pb-0"
                          >
                            <div className="flex items-center gap-2 text-[#5C5F6A] font-inter font-medium text-[12px] capitalize">
                              <span>color:</span>
                              <span
                                className="w-3.5 h-3.5 rounded-full border border-gray-300"
                                style={{
                                  backgroundColor: variant.selectedColor,
                                }}
                              ></span>
                              ...
                              <span>
                                ...
                                <strong className="uppercase">
                                  {variant.selectedSize || "M"}
                                </strong>
                              </span>
                            </div>

                            <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto">
                              <span className="text-[14px] font-bold text-darky shrink-0">
                                {variant.price} EGP
                              </span>
